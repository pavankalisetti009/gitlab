...
module EE
  module Resolvers
    module UserNotesCountResolver # rubocop:disable Gitlab/BoundedContexts -- Needed for override, to be removed after epics migration to work items
      extend ActiveSupport::Concern
      extend ::Gitlab::Utils::Override

      override :load_notes_counts
      def load_notes_counts
        ...
        BatchLoader::GraphQL.for(batch_object).batch do |ids, loader, _args|
          object_counts = ::Note.count_for_collection(
            ids.flat_map(&:first), object.class.base_class.name
          ).index_by(&:noteable_id)

          sync_object_counts = ::Note.count_for_collection(
            ids.flat_map(&:second), object.sync_object.class.base_class.name
          ).index_by(&:noteable_id)
