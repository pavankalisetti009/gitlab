...
module Security
  module Configuration
    class SetGroupValidityChecksService < SetProjectSecuritySettingBaseService
      private
      ...
      def audit
        ...
        audit_context = build_audit_context(
          name: 'group_validity_checks_updated',
          message: message
        )
      ...
      def build_group_message(filtered_out_projects_full_path)
        message = "Validity checks has been #{@enable ? 'enabled' : 'disabled'} for group #{@subject.name} and \
