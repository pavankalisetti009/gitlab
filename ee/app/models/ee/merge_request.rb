  ...
  module MergeRequest
    ...
    prepended do
      ...
      def policy_approval_settings_scan_result_policy_reads
        scan_result_policy_reads_through_violations.includes(approval_policy_rule: :security_policy)
      end
    ...
    def ai_review_merge_request_allowed?(user)
      ::Ai::DuoCodeReview.enabled?(user: user, container: project) && Ability.allowed?(user, :create_note, self)
    end
