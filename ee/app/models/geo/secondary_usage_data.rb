...
module Geo
  class SecondaryUsageData < Geo::TrackingBase
    ...
    def payload_schema_is_valid
      payload.each_key do |key|
        if PAYLOAD_COUNT_FIELDS.include?(key)
          unless payload[key].nil? || payload[key].is_a?(Numeric)
            errors.add(:payload,
              "payload[#{key}] must be a number")
          end
        else
    ...
    def collect_prometheus_metrics
      with_prometheus_client(fallback: nil, verify: false) do |client|
        ...
        self.proxy_remote_requests_event_count_weekly = client.query(PROXY_REMOTE_REQUESTS_EVENT_COUNT_WEEKLY_QUERY).dig(
          0, "value", 1)&.to_i
        self.proxy_local_requests_event_count_weekly = client.query(PROXY_LOCAL_REQUESTS_EVENT_COUNT_WEEKLY_QUERY).dig(0,
          "value", 1)&.to_i
      end
    end
  end
