...
module BillingPlansHelper
  ...
  def plan_feature_list(is_new_trial_type, plan)
    plans_features(is_new_trial_type)[plan.code] || []
  end
  ...
  def plans_features(is_new_trial_type)
    strong_memoize_with(:plans_features, is_new_trial_type) do
      Hashie::Mash.new({
        free: [
        ...
        premium: [
          ...
          *(is_new_trial_type ? [{ title: s_('BillingPlans|GitLab Duo Agent Platform'), highlight: true }] : nil),
          *(is_new_trial_type ? [{ title: s_('BillingPlans|Includes $12 in GitLab Credits per user per month') }] : nil)
        ],
        ultimate: [
          ...
          *(is_new_trial_type ? [{ title: s_('BillingPlans|GitLab Duo Agent Platform'), highlight: true }] : nil),
          *(is_new_trial_type ? [{ title: s_('BillingPlans|Includes $24 in GitLab Credits per user per month') }] : nil)
        ]
      })
    end
  end
