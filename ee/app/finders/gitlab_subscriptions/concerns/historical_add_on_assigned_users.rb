      ...
      def user_ids_with_active_assignments_in_period
        ...
        raw_query = <<~SQL
          WITH latest_assignment_state AS (
            ...
            WHERE namespace_path = {namespace_path:String}
              AND add_on_name = {add_on_name:String}
            GROUP BY user_id, namespace_path, add_on_name, assignment_id
          )
        ...
        ClickHouse::Client.select(query, :main).map { |record| record["uid"] } # rubocop:disable Rails/Pluck -- pluck not suitable for the use case
      end
    end
  end
end
