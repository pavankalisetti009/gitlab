  ...
  describe '#historical_add_on_assigned_users' do
    ...
    context 'with only start_date specified' do
      ...
      before do
        ...
        # Outside the data range and should NOT be included
        clickhouse_fixture(:user_add_on_assignments_history, [
          { assignment_id: 2,
            namespace_path: namespace.traversal_path,
            user_id: member.id,
      ...
      it 'finds users assigned to add on after start_date' do
        expect(assigned_users).to include(member)
      end
    end
    ...
    context 'with only end_date specified' do
      ...
      it 'finds users asssigned to add on before end date' do
        expect(assigned_users).to include(member)
      end
    end
    ...
    context 'with exact namespace path matching' do
      ...
      before do
        clickhouse_fixture(:user_add_on_assignments_history, [
          ...
          { assignment_id: 1,
            namespace_path: parent_namespace.traversal_path,
            user_id: parent_member.id,
            purchase_id: 201,
            add_on_name: 'code_suggestions',
            assigned_at: 10.days.ago,
            revoked_at: nil },
          # Assignment for child namespace (should NOT be included)
          { assignment_id: 2,
            namespace_path: child_namespace.traversal_path,
            user_id: child_member.id,
            purchase_id: 202,
            add_on_name: 'code_suggestions',
            assigned_at: 10.days.ago,
            revoked_at: nil },
          # Assignment for similar namespace (should NOT be included)
          { assignment_id: 3,
            namespace_path: similar_namespace.traversal_path,
      ...
      it 'only includes users from the exact namespace' do
        expect(assigned_users).to contain_exactly(parent_member)
      end

      it 'does not include users from child namespaces' do
        expect(assigned_users).not_to include(child_member)
      end

      it 'does not include users from namespaces with similar paths' do
        expect(assigned_users).not_to include(similar_member)
      end

      context 'when querying for child namespace' do
        subject(:child_assigned_users) do
          GitlabSubscriptions::AddOnAssignedUsersFinder.new(
            user,
            child_namespace,
            **finder_params
        ...
        it 'only includes users from the child namespace' do
          expect(child_assigned_users).to contain_exactly(child_member)
        end

        it 'does not include users from parent namespace' do
          expect(child_assigned_users).not_to include(parent_member)
        end
      end
    end
