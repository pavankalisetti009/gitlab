...
RSpec.describe Gitlab::Checks::SecretPushProtection::ExclusionsManager, feature_category: :secret_detection do
  ...
  describe '#active_exclusions' do
    context 'when exclusions are available (Ultimate license)' do
      before do
    ...
    context 'when exclusions are not available (public .com project without Ultimate)' do
      before do
        ...
        allow(project).to receive(:licensed_feature_available?).with(:secret_push_protection).and_return(false)
      end
      ...
      it 'returns an empty hash' do
        expect(exclusions_manager.active_exclusions).to eq({})
      end

      it 'does not query the database for exclusions' do
        expect(project).not_to receive(:security_exclusions)

        exclusions_manager.active_exclusions
      end
    end
  end
  ...
  describe '#matches_excluded_path?' do
    ...
    context 'when exclusions are not available (public .com project)' do
      before do
        project.update!(visibility_level: Gitlab::VisibilityLevel::PUBLIC)
        stub_saas_features(auto_enable_secret_push_protection_public_projects: true)
        stub_feature_flags(auto_spp_public_com_projects: true)
      ...
      it 'returns false even when exclusion would match' do
        expect(exclusions_manager.matches_excluded_path?('test.rb')).to be(false)
        expect(audit_logger).not_to have_received(:log_exclusion_audit_event)
      end

      it 'does not check exclusions' do
