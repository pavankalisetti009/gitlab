...
RSpec.describe 'Multi-Container-Scanning.gitlab-ci.yml', feature_category: :container_scanning do
  include Ci::PipelineMessageHelpers

  subject(:template) { Gitlab::Template::GitlabCiYmlTemplate.find('Multi-Container-Scanning.latest') }
  ...
  describe 'the created pipeline' do
    ...
    context 'for all license tiers' do
      shared_examples 'common pipeline checks' do
        include_examples 'has expected jobs', %w[multi-cs::generate-scan]
        include_examples 'has jobs that can be disabled',
          'CONTAINER_SCANNING_DISABLED', %w[true 1], %w[multi-cs::generate-scan]
      end
