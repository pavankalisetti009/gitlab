...
RSpec.describe RemoteDevelopment::WorkspaceOperations::Reconcile::Output::DesiredConfigGenerator, "Golden Master for desired_config", feature_category: :workspaces do
  ...
  shared_examples "generated desired_config golden master checks" do
    it "exactly matches the generated desired_config", :unlimited_max_formatted_output_length do
      desired_config_sorted =
        desired_config
          .map(&:deep_symbolize_keys)
          .sort_by { |resource| [resource.fetch(:kind), resource.fetch(:metadata).fetch(:name)] }
          .map { |hash| Gitlab::Utils.deep_sort_hash(hash) }

      golden_master_desired_config_sorted =
      ...
      desired_config_sorted.each_with_index do |resource, index|
        resource_differences = Hashdiff.diff(resource, golden_master_desired_config_sorted[index])
  ...
  # @return [String]
  def input_processed_devfile_yaml
    <<~YAML
      ---
      schemaVersion: 2.2.0
      metadata: {}
      components:
        ...
        - name: gl-workspace-data
          volume:
            size: 50Gi
      commands:
        - id: gl-tooling-container-command
          exec:
            commandLine: "echo 'tooling container command'"
