...
RSpec.describe Search::Zoekt::Features::TraversalIdSearch, feature_category: :global_search do
  ...
  describe '#preflight_checks_passed?' do
    ...
    before do
      ...
      allow(::Search::Zoekt::Settings).to receive(:minimum_projects_for_traversal_id_search).and_return(threshold)
    end

    context 'when threshold is 1 or less' do
      let(:threshold) { 1 }
    ...
    context 'when zoekt_disable_large_project_checks feature flag is enabled for the namespace' do
      before do
        stub_feature_flags(zoekt_disable_large_project_checks: namespace)
      end

      context 'with project count below threshold' do
    ...
    context 'when zoekt_disable_large_project_checks feature flag is disabled for the namespace' do
      before do
        stub_feature_flags(zoekt_disable_large_project_checks: false)
      end
      ...
      context 'when there is a project_id specified' do
        let(:feature) { described_class.new(nil, project_id: 1) }

        it { is_expected.to be false }
      end

      context 'when there is a global search' do
        let(:feature) { described_class.new(nil) }
