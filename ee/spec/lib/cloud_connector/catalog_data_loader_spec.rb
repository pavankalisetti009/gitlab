...
RSpec.describe CloudConnector::CatalogDataLoader, feature_category: :cloud_connector do
  ...
  describe '#initialize' do
    it 'initializes with a model class and gets a loader from the factory' do
      expect(CloudConnector::DataLoaderFactory).to receive(:create_loader).with(model_class)

      described_class.new(model_class)
    end
  end
  ...
  it 'inherits from AbstractDataLoader' do
    expect(described_class).to be < Gitlab::CloudConnector::DataModel::AbstractDataLoader
  end
end
