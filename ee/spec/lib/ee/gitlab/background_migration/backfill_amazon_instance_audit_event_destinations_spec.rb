...
RSpec.describe Gitlab::BackgroundMigration::BackfillAmazonInstanceAuditEventDestinations,
  ...
  let!(:legacy_destination2) do
    legacy_table.create!(
      name: "Second AWS S3 Instance Bucket",
      access_key_xid: 'b' * 32,
      bucket_name: "second-instance-bucket",
  ...
  let!(:streaming_destination) do
    streaming_table.create!(
      ...
      config: { 'accessKeyXid' => 'c' * 32, 'bucketName' => 'migrated-bucket', 'awsRegion' => 'eu-central-1' }.to_json,
  ...
  let!(:migrated_legacy_destination) do
    legacy_table.create!(
      name: "Already Migrated AWS S3 Instance Bucket",
      access_key_xid: 'c' * 32,
  ...
  describe '#perform' do
    it 'creates streaming destinations for unmigrated records and updates them with correct encryption keys' do
      ...
      config1 = ::Gitlab::Json.parse(new_streaming_dest1.config)
    ...
    it 'only processes records with nil stream_destination_id' do
      reference_streaming = streaming_table.create!(
        ...
        config: { 'accessKeyXid' => 'd' * 32, 'bucketName' => 'reference-bucket', 'awsRegion' => 'us-east-1' }.to_json,
