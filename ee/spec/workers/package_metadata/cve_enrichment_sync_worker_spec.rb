...
RSpec.describe PackageMetadata::CveEnrichmentSyncWorker, type: :worker, feature_category: :software_composition_analysis do
  describe '#perform' do
    ...
    shared_examples_for 'it syncs' do
      it 'calls sync service with the cve_enrichment data_type' do
        expect(PackageMetadata::SyncService).to receive(:execute)
          .with(data_type: 'cve_enrichment', lease: lease)
    ...
    context 'when the dependency_scanning feature is disabled' do
      before do
    ...
    context 'when the dependency_scanning feature is enabled' do
      before do
