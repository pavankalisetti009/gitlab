...
RSpec.describe VirtualRegistries::Container::Cache::MarkEntriesForDestructionWorker, feature_category: :virtual_registry do
  ...
  describe '#perform' do
    context 'when the upstream is found' do
      ...
      before do
        create_list(:virtual_registries_container_cache_entry, 3, upstream:) # 3 default
      ...
      it 'marks default cache entries for destruction' do
        expect { perform }.to change {
          ::VirtualRegistries::Container::Cache::Entry.pending_destruction.size
        }.by(3)
      end
    end
    ...
    context 'when the upstream is not found' do
      ...
      it 'does not mark any cache entries for destruction' do
        expect { perform }.not_to change { ::VirtualRegistries::Container::Cache::Entry.count }
