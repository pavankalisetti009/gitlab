...
RSpec.describe VirtualRegistries::Cache::DestroyOrphanEntriesWorker, feature_category: :virtual_registry do
  ...
  describe '#perform_work' do
    ...
    context 'with work to do' do
      ...
      context 'when an error occurs during destruction' do
        before do
          allow_next_found_instance_of(model) do |instance|
            allow(instance).to receive(:destroy!).and_raise(StandardError, 'Test error')
          end
        end

        it 'logs the exception with correct metadata' do
          expect(::Gitlab::ErrorTracking).to receive(:log_exception).with(
        ...
        it 'updates the status to error when record is not destroyed' do
          expect(Gitlab::ErrorTracking).to receive(:log_exception)
            .with(instance_of(StandardError), class: described_class.name)
  ...
  describe '#remaining_work_count' do
    ...
    context 'with no orphan entries' do
