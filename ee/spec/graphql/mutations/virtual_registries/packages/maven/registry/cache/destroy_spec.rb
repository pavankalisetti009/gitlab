...
RSpec.describe Mutations::VirtualRegistries::Packages::Maven::Registry::Cache::Destroy, :sidekiq_inline, feature_category: :virtual_registry do
  ...
  let_it_be(:cache2) { create_list(:virtual_registries_packages_maven_cache_remote_entry, 2) }
  ...
  describe '#resolve' do
    ...
    context 'when the user does not have permission to purge a registries cache' do
      it_behaves_like 'raises resource not available error'
    end

    context 'when the user has permission to purge a registries cache' do
      before_all do
