...
RSpec.describe 'AI events backfill', :freeze_time, :click_house, :sidekiq_inline,
  ...
  context 'when application setting was already enabled' do
    before do
      ...
      ClickHouse::Client.execute("TRUNCATE #{Ai::DuoChatEvent.clickhouse_table_name}", :main)
      ClickHouse::Client.execute("TRUNCATE #{Ai::CodeSuggestionEvent.clickhouse_table_name}", :main)
    end
