...
RSpec.describe 'Fetching Groups and Projects for CI_JOB_TOKEN', feature_category: :continuous_integration do
  ...
  let_it_be(:subgroup1) { create(:group, parent: group, name: 'fruit') }
  let_it_be(:subgroup2) { create(:group, parent: subgroup1, name: 'fruit') }
  ...
  let_it_be(:subgroup6) { create(:group, parent: subgroup5, name: 'vegetable') }

  let_it_be(:project1) { create(:project, group: subgroup1, name: 'banana', path: 'banana') }
  let_it_be(:project2) { create(:project, group: subgroup2, name: 'banana', path: 'banana') }
  let_it_be(:project3) { create(:project, group: subgroup3, name: 'banana', path: 'banana') }
  let_it_be(:project4) { create(:project, group: subgroup4, name: 'banana', path: 'banana') }
  let_it_be(:project5) { create(:project, group: subgroup5, name: 'banana', path: 'banana') }
  let_it_be(:project6) { create(:project, group: subgroup6, name: 'cabbage') }
  ...
  describe 'Get groups and projects query for CI_JOB_TOKEN' do
    ...
    context 'when searching project by name' do
      ...
      it 'searches namespaces for exact match' do
        ...
        expect(fetched_projects_data['nodes'].first['id']).to eq "gid://gitlab/Project/#{project6.id}"
      end
    end

    context 'when searching project by path' do
      before do
        get_graphql(project_query('fruit/banana'), current_user: user)
      end
      ...
      it 'sorts by id (ascending order) when there are multiple matches' do
        ...
        expect(fetched_projects_data['nodes'].pluck('id')).to eq [
          "gid://gitlab/Project/#{project1.id}",
          "gid://gitlab/Project/#{project2.id}",
    ...
    context 'when searching group by name' do
      ...
      it 'finds exact match' do
        ...
        expect(fetched_groups_data['nodes'].first['id']).to eq "gid://gitlab/Group/#{subgroup6.id}"
      end
    end
    ...
    context 'when searching group by path' do
      ...
      it 'sorts by id (ascending order) when there are multiple matches' do
        expect(fetched_groups_data['nodes'].length).to eq 4
        expect(fetched_groups_data['nodes'].pluck('id')).to eq [
          "gid://gitlab/Group/#{subgroup2.id}",
          "gid://gitlab/Group/#{subgroup3.id}",
  ...
  def project_query(search)
    <<~QUERY
    query {
      projects(search: "#{search}", searchNamespaces: true, sort: "id_asc", first: 4) {
        nodes {
          id
        }
      }
