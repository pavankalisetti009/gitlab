...
RSpec.describe 'Querying for webhook events', feature_category: :webhooks do
  ...
  describe 'webhook events on a group hook' do
    let_it_be(:group_hook) { create(:group_hook) }
    let_it_be(:user) { create(:user) }
    let_it_be(:current_user) { user }
    let_it_be(:webhook_events) { create_list(:web_hook_log, 5, web_hook: group_hook) }
    ...
    context 'when the user is authorized' do
      ...
      it 'returns webhook events' do
        response_ids = graphql_data_at('group', 'webhook', 'webhookEvents', 'nodes', 'id')
        expected_ids = group_hook.web_hook_logs.map { |web_hook_log| GitlabSchema.id_from_object(web_hook_log).to_s }
    ...
    context 'when the user is not authorized' do
      ...
      it 'does not return webhook events' do
        response_id = graphql_data_at('group', 'webhook', 'webhookEvents')
