...
RSpec.describe 'Destroy an upstream registry', feature_category: :virtual_registry do
  ...
  let(:mutation_response) { graphql_mutation_response(:maven_upstream_delete) }

  def maven_upstream_mutation(params = mutation_params)
    graphql_mutation(:mavenUpstreamDelete, params)
  end
  ...
  it 'destroys the maven upstream registry' do
  ...
  context 'when mutation params are invalid' do
    it_behaves_like 'returning error message' do
      let(:mutation) { maven_upstream_mutation(faulty_mutation_params) }
      let(:error_msg) do
        "Variable $mavenUpstreamDeleteInput of type MavenUpstreamDeleteInput! " \
  ...
  it_behaves_like 'returning error message' do
    let(:mutation) { maven_upstream_mutation(invalid_record_params) }
  end
  ...
  context 'with maven_virtual_registry feature flag turned off' do
    ...
    it_behaves_like 'returning error message' do
      let(:mutation) { maven_upstream_mutation }
    end
  end
end
