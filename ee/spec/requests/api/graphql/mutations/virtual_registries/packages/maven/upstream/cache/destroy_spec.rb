...
RSpec.describe 'Destroy a Maven upstream cache', :sidekiq_inline, feature_category: :virtual_registry do
  ...
  let_it_be(:cache) { create_list(:virtual_registries_packages_maven_cache_remote_entry, 2, upstream: upstream) }
  ...
  def upstream_cache_delete_mutation(params = mutation_params)
    graphql_mutation(:mavenUpstreamCacheDelete, params)
  end
  ...
  it 'destroys the cache' do
    expect do
      post_graphql_mutation(upstream_cache_delete_mutation, current_user: current_user)
    end.to change { ::VirtualRegistries::Packages::Maven::Cache::Remote::Entry.pending_destruction.count }.by(2)
  end

  context 'when mutation params are invalid' do
    it_behaves_like 'returning error message' do
      let(:mutation) { upstream_cache_delete_mutation(faulty_mutation_params) }
      let(:error_msg) do
        "Variable $mavenUpstreamCacheDeleteInput of type MavenUpstreamCacheDeleteInput! " \
  ...
  it_behaves_like 'returning error message' do
    let(:mutation) { upstream_cache_delete_mutation(invalid_record_params) }
  end
  ...
  context 'when packages_virtual_registry licensed feature is unavailable' do
    ...
    it_behaves_like 'returning error message' do
      let(:mutation) { upstream_cache_delete_mutation }
    end
  end
