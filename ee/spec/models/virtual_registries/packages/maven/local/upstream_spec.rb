...
RSpec.describe VirtualRegistries::Packages::Maven::Local::Upstream, feature_category: :virtual_registry do
  ...
  let_it_be(:other_project, freeze: true) { create(:project) }
  let_it_be(:other_group, freeze: true) { create(:group) }
  ...
  describe 'validations' do
    ...
    context 'with a local group upstream' do
      subject(:local_group_upstream) do
        build(:virtual_registries_packages_maven_local_upstream, :local_group, group: other_group)
      end
  ...
  describe 'scopes' do
    describe '.for_id_and_group' do
      let_it_be(:upstream, freeze: true) { create(:virtual_registries_packages_maven_local_upstream) }
      let_it_be(:other_upstream, freeze: true) { create(:virtual_registries_packages_maven_local_upstream) }
    ...
    describe '.for_group' do
      ...
      let_it_be(:upstream, freeze: true) { create(:virtual_registries_packages_maven_local_upstream, group:) }
      let_it_be(:other_upstream, freeze: true) { create(:virtual_registries_packages_maven_local_upstream) }
    ...
    describe '.search_by_name' do
      ...
      let_it_be(:other_upstream, freeze: true) { create(:virtual_registries_packages_maven_local_upstream) }
      let_it_be(:upstream, freeze: true) do
        create(:virtual_registries_packages_maven_local_upstream, name: 'pkg-name-abc')
      end
