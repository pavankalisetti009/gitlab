...
RSpec.describe GitlabSubscriptions::DuoEnterprise, feature_category: :subscription_management do
  ...
  describe '.any_pro_enterprise_add_on_purchase_for_namespace' do
    let_it_be(:namespace) { create(:namespace) }
    ...
    context 'when there is an add-on purchase for the namespace' do
      ...
      it 'returns the add-on purchase' do
        expect(described_class.any_pro_enterprise_add_on_purchase_for_namespace(namespace).id).to eq(add_on_purchase.id)
      end
    end
    ...
    context 'when there is no add-on purchase for the namespace' do
      it 'returns nil' do
        expect(described_class.any_pro_enterprise_add_on_purchase_for_namespace(namespace)).to be_nil
      end
    end
  end
end
