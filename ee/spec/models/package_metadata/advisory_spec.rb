...
RSpec.describe PackageMetadata::Advisory, type: :model, feature_category: :software_composition_analysis do
  ...
  # Add this describe block at the end of the existing spec file

  describe '.by_cve' do
    let_it_be(:advisory_with_cve) do
      create(:pm_advisory, identifiers: [
        create(:pm_identifier, :cve, name: 'CVE-2023-12345'),
        create(:pm_identifier, type: "ghsa", name: "GHSA-9445-4cr6-336r", value: "GHSA-9445-4cr6-336r")
      ])
    end

    let_it_be(:advisory_with_odd_type_case_cve) do
      create(:pm_advisory, identifiers: [
        create(:pm_identifier, type: 'CVe', name: 'CVE-2023-67890', value: 'CVE-2023-67890')
      ])
    end

    let_it_be(:advisory_without_cve) do
      create(:pm_advisory, identifiers: [
        create(:pm_identifier, type: 'ghsa', name: 'GHSA-wxyz-9876-5432', value: 'GHSA-wxyz-9876-5432')
      ])
    end

    it 'returns the advisories matching the given CVE' do
      result = described_class.by_cve('CVE-2023-12345')
      expect(result).to eq([advisory_with_cve])
    end

    it 'is case-insensitive for the CVE type' do
      result = described_class.by_cve('CVE-2023-67890')
      expect(result).to eq([advisory_with_odd_type_case_cve])
    end

    it 'returns nil when no matching CVE is found' do
      result = described_class.by_cve('CVE-2023-99999')
      expect(result).to eq([])
    end

    it 'returns nil when the advisory has no CVE identifiers' do
      result = described_class.by_cve('GHSA-wxyz-9876-5432')
      expect(result).to eq([])
    end

    it 'returns multiple matching advisories' do
      create(:pm_advisory, identifiers: [
        create(:pm_identifier, :cve, name: 'CVE-2023-12345')
      ])
      ...
      expect(described_class.by_cve('CVE-2023-12345').count).to eq(2)
    end
  end
end
