...
RSpec.describe Vulnerabilities::Finding, feature_category: :vulnerability_management do
  ...
  with_them do
    ...
    describe '#cve_enrichment' do
      ...
      before do
        allow(finding).to receive(:cve_value).and_return(cve_value)
      end
    ...
    describe '#advisory' do
      ...
      let_it_be(:advisory) do
        create(:pm_advisory, identifiers: [
          create(:pm_identifier, :cve, name: cve_value)
        ])
      end
      ...
      it 'returns the Advisory for the findings CVE' do
        expect(finding.advisory).to eq(advisory)
      end

      it 'memoizes the result' do
        expect(PackageMetadata::Advisory).to receive(:by_cve).once.and_return([advisory])
        2.times { finding.advisory }
      end

      context 'when no Advisory is found' do
        it 'returns nil' do
          expect(PackageMetadata::Advisory).to receive(:by_cve).with(cve_value).and_return([])
          expect(finding.advisory).to be_nil
        end
      end

      context 'when two advisories are found' do
