...
RSpec.describe GitlabSubscriptions::AddOnPurchases::SelfManaged::LicenseAddOns::DuoEnterprise,
  ...
  describe '#execute' do
    ...
    let(:restrictions) do
      {
        add_on_products: {
          "duo_enterprise" => [{ "quantity" => 1 }, { "quantity" => 2 }]
        }
      }
    end
    ...
    describe "#seat_count" do
      ...
      context "with empty duo enterprise info" do
        let(:restrictions) do
          {
            add_on_products: {
              "duo_pro" => [{ "quantity" => 5 }]
      ...
      context "with an empty quantity key in the duo enterprise info" do
        let(:restrictions) do
          {
            add_on_products: {
              "duo_enterprise" => [{ "started_on" => '2024-08-01' }]
            }
          }
        end
