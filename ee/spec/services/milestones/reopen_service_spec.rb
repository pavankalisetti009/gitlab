...
RSpec.describe Milestones::ReopenService, feature_category: :team_planning do
  ...
  describe '#execute' do
    context 'on group milestones' do
      ...
      let(:milestone) { create(:milestone, :closed, title: 'Milestone v1.0', group: container) }
      ...
      context 'when group webhooks are available' do
        ...
        context 'when group has active milestone hooks' do
          ...
          it_behaves_like 'reopens the milestone', with_hooks: true, with_event: false
        end

        context 'when group has no active milestone hooks' do
          it_behaves_like 'reopens the milestone', with_hooks: false, with_event: false
        end
      end
