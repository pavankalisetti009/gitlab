...
RSpec.describe ::VirtualRegistries::Container::CreateRegistryService, feature_category: :virtual_registry do
  ...
  describe '#execute' do
    ...
    it 'creates a registry successfully' do
      expect { result }.to change {
        ::VirtualRegistries::Container::Registry.where(group: group).count
      }.from(0).to(1)
    end
    ...
    context 'when max registries per group is reached' do
      before_all do
        create_list(:virtual_registries_container_registry,
