...
RSpec.describe SecretsManagement::ProjectSecretsManagers::InitiateDeprovisionByPathService,
  ...
  let(:params) do
    {
      namespace_path: SecretsManagement::ProjectSecretsManager.build_namespace_path(project.namespace),
  ...
  describe '#execute' do
    let(:deprovision_worker_spy) { class_spy(SecretsManagement::DeprovisionProjectSecretsManagerByPathWorker) }

    before do
      stub_const('SecretsManagement::DeprovisionProjectSecretsManagerByPathWorker', deprovision_worker_spy)
    end
    ...
    context 'when secrets manager exists and is active' do
      ...
      it 'initiates the deprovision process by path', :aggregate_failures do
        ...
        expect(deprovision_worker_spy).to have_received(:perform_async)
          .with(user.id, secrets_manager.id, params[:namespace_path], params[:project_path])
      end
    end
