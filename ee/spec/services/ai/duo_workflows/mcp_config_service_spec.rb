...
RSpec.describe Ai::DuoWorkflows::McpConfigService, feature_category: :duo_agent_platform do
  ...
  describe '#execute' do
    ...
    it 'includes enabled tools list' do
      ...
      expect(result[:gitlab][:Tools]).to eq(%w[gitlab_search semantic_code_search])
    end

    it 'includes preapproved tools list' do
      result = service.execute

      expect(result[:gitlab][:PreApprovedTools]).to eq(%w[gitlab_search])
    end
  ...
  describe '#gitlab_enabled_tools' do
    it 'returns array of enabled tools' do
      ...
      expect(result).to eq(%w[gitlab_search semantic_code_search])
    end
  ...
  describe 'constant GITLAB_ENABLED_TOOLS' do
    it 'is defined with expected tools' do
      expect(described_class::GITLAB_ENABLED_TOOLS).to eq(%w[gitlab_search semantic_code_search])
    end
  end

  describe 'constant GITLAB_PREAPPROVED_TOOLS' do
    it 'is defined with expected tools' do
      expect(described_class::GITLAB_PREAPPROVED_TOOLS).to eq(%w[gitlab_search])
    end
  end
