  ...
  describe '#available?' do
    ...
    shared_examples 'duo_foundational_flows_enabled check' do
      context 'when duo_foundational_flows_enabled is false' do
        before do
          allow(::Gitlab::Llm::StageCheck).to receive(:available?).with(resource, :duo_workflow).and_return(true)
    ...
    shared_examples 'code_review flow not enabled check' do
    ...
    shared_examples 'Duo Pro/Core behavior' do
      ...
      context 'when user has Duo Pro/Core add-on' do
        ...
        it_behaves_like 'code_review flow not enabled check'

        context 'when code_review flow is enabled' do
          let(:namespace_for_flow) { resource.is_a?(Project) ? resource.root_ancestor : resource }
          ...
          before do
            ...
            create(
              :ai_catalog_enabled_foundational_flow,
              :for_namespace,
              namespace: namespace_for_flow,
