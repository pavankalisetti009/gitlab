...
RSpec.describe CloudConnector::StatusChecks::StatusService, feature_category: :cloud_connector do
  ...
  subject(:service) { described_class.new(probes: probes) }
  ...
  describe '#execute' do
    ...
    context 'when any probe fails' do
      ...
      it 'executes all probes and returns unsuccessful status result' do
        expect(succeeded_probe).to receive(:execute).once.and_call_original
        expect(failed_probe).to receive(:execute).once.and_call_original
