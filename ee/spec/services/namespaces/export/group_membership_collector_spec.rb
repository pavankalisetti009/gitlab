...
RSpec.describe Namespaces::Export::GroupMembershipCollector, feature_category: :system_access do
  ...
  describe '#execute' do
    ...
    def mock_group(group, members, _parent_groups)
      ...
      combinator = instance_double(Namespaces::Export::GroupMembersTypeCombinator)

      expect(Namespaces::Export::GroupMembersTypeCombinator).to receive(:new).with(group).and_return(combinator)
    ...
    it 'returns correct data' do
      ...
      expect(result.map(&:id)).to eq([group_owner_1.id,
        sub_group_1_owner_2.id, group_owner_1.id, sub_sub_group_owner_4.id, shared_maintainer_5.id])
    end
  end
end
