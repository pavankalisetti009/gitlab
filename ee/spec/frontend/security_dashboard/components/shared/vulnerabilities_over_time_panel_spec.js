...
describe('VulnerabilitiesOverTimePanel', () => {
  ...
  const createComponent = ({
    ...
    wrapper = shallowMountExtended(VulnerabilitiesOverTimePanel, {
      ...
      provide: {
        projectFullPath: scopeConfigs.project.fullPath,
        groupFullPath: scopeConfigs.group.fullPath,
      },
    });
  ...
  describe.each(['project', 'group'])('%s scope', (scopeType) => {
    ...
    describe('data fetching', () => {
      it(`fetches data with correct variables for ${scopeType} namespace`, async () => {
      ...
      it(`handles projectId correctly for ${scopeType}`, async () => {
        const { vulnerabilitiesOverTimeHandler, config } = createComponent({ scope: scopeType });
        await waitForPromises();
      ...
      it('re-fetches data when time period changes', async () => {
        const { vulnerabilitiesOverTimeHandler } = createComponent({ scope: scopeType });
        await waitForPromises();
    ...
    describe('error handling', () => {
      it('shows error state when GraphQL query fails', async () => {
        createComponent({
          scope: scopeType,
          mockVulnerabilitiesOverTimeHandler: jest
            .fn()
