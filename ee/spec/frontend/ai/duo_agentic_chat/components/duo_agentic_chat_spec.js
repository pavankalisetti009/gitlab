...
import { MOCK_AI_CHAT_AVAILABLE_MODELS_RESPONSE, MOCK_MODEL_LIST_ITEMS } from './mock_data';
  ...
  describe('Agentic chat user model selection', () => {
    ...
    describe('when user model selection is enabled', () => {
      beforeEach(() => {
        duoChatGlobalState.isAgenticChatShown = true;
        createComponent({
          propsData: { userModelSelectionEnabled: true, namespaceId: MOCK_NAMESPACE_ID },
        });
      });
      ...
      it('updat the props to `ModelSelectDropdown`', async () => {
        await waitForPromises();

        expect(findModelSelectDropdown().props('placeholderDropdownText')).toBe('Select a model');
        // The component uses mockListItems from model selection, not the GraphQL response
    ...
    it.each`
      case                                       | isAgenticChatShown | userModelSelectionEnabled | hasNamespaceId
      ${'when user model selection is disabled'} | ${true}            | ${false}                  | ${true}
      ${'when isAgenticChatShown is false'}      | ${false}           | ${true}                   | ${true}
      ${'when no namespace ID is passed'}        | ${true}            | ${true}                   | ${false}
    `(
      'when $case: query is skipped',
      ({ isAgenticChatShown, userModelSelectionEnabled, hasNamespaceId }) => {
        beforeEach(() => {
          duoChatGlobalState.isAgenticChatShown = isAgenticChatShown;
          createComponent({
            propsData: {
              userModelSelectionEnabled,
              namespaceId: hasNamespaceId ? MOCK_NAMESPACE_ID : undefined,
            },
          });
        });
