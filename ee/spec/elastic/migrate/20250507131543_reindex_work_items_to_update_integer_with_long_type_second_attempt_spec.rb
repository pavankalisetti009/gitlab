...
require File.expand_path(
  'ee/elastic/migrate/20250507131543_reindex_work_items_to_update_integer_with_long_type_second_attempt.rb'
)

RSpec.describe ReindexWorkItemsToUpdateIntegerWithLongTypeSecondAttempt, feature_category: :global_search do
  let(:version) { 20250507131543 }
  let(:migration) { described_class.new(version) }
  let(:task) { Search::Elastic::ReindexingTask.last }
  let(:targets) { %w[WorkItem] }
