...
RSpec.describe GitlabSubscriptions::Trials::DuoPro::LegacyTrialFormWithErrorsComponent, :saas, :aggregate_failures, feature_category: :acquisition do
  let(:errors) { ['some CDOT error'] }
  let(:reason) { nil }
  let(:additional_kwargs) { { errors: errors, reason: reason } }

  it_behaves_like GitlabSubscriptions::Trials::DuoPro::LegacyTrialFormComponent do
