...
RSpec.describe 'Groups > Usage quotas > Pipelines tab', :js, :saas, feature_category: :consumables_cost_management do
  ...
  context 'with compute over quota' do
    ...
    context 'in usage by project' do
      ...
      context 'when group has projects in subgroups' do
        ...
        it 'shows projects inside the subgroup',
          quarantine: "https://gitlab.com/gitlab-org/quality/test-failure-issues/-/issues/24087" do
          visit_usage_quotas_page
  ...
  context 'with pagination', quarantine: "https://gitlab.com/gitlab-org/quality/test-failure-issues/-/issues/24087" do
    include_context 'when user is allowed to see usage quotas'
  ...
  context 'when user has limited access to subscription', :with_trial_types do
    before do
      stub_signing_key
