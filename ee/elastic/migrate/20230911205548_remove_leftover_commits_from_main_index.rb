...
class RemoveLeftoverCommitsFromMainIndex < Elastic::Migration
  ...
  def migrate
    ...
    if task_id
      ...
      if task_status['error'].present?
        log_raise 'Failed to delete commits', task_id: task_id, failures: task_status['error']
      end

      if task_status['completed']
        log 'Removing commits from the original index is completed for a specific task', task_id: task_id
        set_migration_state(task_id: nil, documents_remaining: original_documents_count)
      else
        log 'Removing commits from the original index is still in progress for a specific task', task_id: task_id
      end
    ...
    if response['failures'].present?
      log_raise 'Failed to delete commits', task_id: task_id, failures: response['failures']
    end

    task_id = response['task']
    log 'Removing commits from the original index is started for a task', task_id: task_id
