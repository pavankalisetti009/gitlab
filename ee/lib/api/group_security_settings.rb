    ...
    resource :groups, requirements: ::API::API::NAMESPACE_OR_PROJECT_REQUIREMENTS do
      segment ':id/security_settings' do
        desc 'Update group security settings' do
          detail 'Updates pre_receive_secret_detection_enabled for all projects to the new value'
          tags %w[groups]
        end
        params do
          requires :secret_push_protection_enabled, type: Boolean, desc: 'Whether to enable the feature'
          optional :projects_to_exclude, type: Array[Integer], desc: 'IDs of projects to exclude from the feature'
        end
        put do
          unauthorized! unless can?(current_user, :enable_pre_receive_secret_detection, user_group)
