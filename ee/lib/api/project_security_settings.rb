  ...
  class ProjectSecuritySettings < ::API::Base
    ...
    helpers do
      def check_feature_availability
        forbidden! unless ::License.feature_available?(:pre_receive_secret_detection)
      end
    end
    ...
    resource :projects, requirements: ::API::API::NAMESPACE_OR_PROJECT_REQUIREMENTS do
      segment ':id/security_settings' do
        ...
        desc 'Update project security settings' do
          detail 'Updates pre_receive_secret_detection_enabled to the new value & returns new project security setting'
          tags %w[projects]
        end
        params do
          requires :pre_receive_secret_detection_enabled, type: Boolean, desc: 'Enable/disable secret push protection'
        end
        put do
          unauthorized! unless can?(current_user, :manage_security_settings, user_project)
          enabled = params[:pre_receive_secret_detection_enabled]
          audit_context = {
            name: 'project_security_setting_updated',
            author: current_user,
            target: user_project,
            scope: user_project,
            message: "User #{current_user.name} updated `pre_receive_secret_detection_enabled` to #{enabled}"
          }

          ::Gitlab::Audit::Auditor.audit(audit_context)
          security_setting = user_project&.security_setting
          security_setting.set_pre_receive_secret_detection!(enabled: enabled)
