...
module Search
  module Elastic
    class Pagination
      ...
      def initialize(query_hash, tie_breaker_property = :id)
        @query_hash = query_hash.deep_dup
        @sort_property_value = nil
        @tie_breaker_property = tie_breaker_property
              ...
              should: [
                {
                  range: {
                    sort_property => {
                      document_matching_operator => sort_property_value
                    }
                  }
                },
                {
                      ...
                      },
                      {
                        range: {
                          tie_breaker_property => {
                            document_matching_operator => tie_breaker_property_value
                          }
                        }
                      }
                    ]
      ...
      def reverse_sort_direction
        original_sort_direction == :asc ? :desc : :asc
      end
      ...
      def document_matching_operator
        if original_sort_direction == :asc
          is_after ? :gt : :lt
        else
          is_after ? :lt : :gt
        end
