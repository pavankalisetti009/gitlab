      ...
      module Database
        class Wrapper
          ...
          def all_database_connection_files
            Dir.glob(context.gitlab_basepath.join("db/database_connections/*.yaml"))
          end

          def all_gitlab_schema_files
            Dir.glob(context.gitlab_basepath.join("db/gitlab_schemas/*.yaml"))
          end

          def all_database_connections
            @all_database_connections ||=
              all_database_connection_files
                .map { |file| ConnectionInfo.load_file(file) }
                .sort_by(&:order)
                .index_by(&:name)
          ...
          # tables.
          def database_base_models_with_gitlab_shared
          ...
          # healthy
          def upgrade_path
            path_data = YAML.safe_load_file(context.gitlab_basepath.join('config/upgrade_path.yml'))
            Gitlab::Utils::UpgradePath.new(path_data, Gitlab.version_info)
          end

          # Migrations before this version may have been removed.
          # def min_schema_gitlab_version
          #   upgrade_path.last_required_stop
          # end

          # Database configured. Returns true even if the database is shared
          def has_config?(database_name)
            true
            # TODO
            # ActiveRecord::Base.configurations
            #   .configs_for(env_name: context.env, name: database_name.to_s, include_hidden: true)
            #   .present?
          end

          # Database configured. Returns false if the database is shared
          def has_database?(database_name)
            db_config = database_base_models[database_name]&.connection_db_config
            db_config.present? && db_config_share_with(db_config).nil?
          end

          def multiple_databases?
            database_mode == MODE_MULTIPLE_DATABASES
          end
          ...
          #     path = context.gitlab_basepath.join(db_path, 'post_migrate').to_s
          ...
          # def db_config_names(with_schema:)
          ...
          # def gitlab_schemas_for_connection(connection)
          ...
          #   primary_model = database_base_models.fetch(db_config_name.to_sym)

          #   schemas_to_base_models.select do |_, child_models|
          ...
          #         database_base_models[db_config_share_with(child_model.connection_db_config)] == primary_model
          #     end
          #   end.keys.map!(&:to_sym)
          # end

          def db_config_for_connection(connection)
          ...
          # def db_config_name(connection)
          #   db_config = db_config_for_connection(connection)
          #   db_config&.name || 'unknown'
          # end

          # def db_config_database(connection)
          #   db_config = db_config_for_connection(connection)
          ...
          def db_config_share_with(db_config)
            # no sharing
          ...
          # def read_minimum_migration_version
          #   Dir.open(
          #     context.gitlab_basepath.join('db/migrate')
          #   ).filter_map { |f| /\A\d{14}/.match(f)&.to_s }.map(&:to_i).min
          # end

          # Monkeypatch rails with upgraded database observability
          # def install_transaction_metrics_patches!
          #   ActiveRecord::Base.prepend(ActiveRecordBaseTransactionMetrics)
          # end

          # def install_transaction_context_patches!
          ...
          module ActiveRecordBaseTransactionMetrics
            ...
            class_methods do
              ...
              #   ::Gitlab::Database::Metrics.subtransactions_increment(name) if transaction_type == :sub_transaction
