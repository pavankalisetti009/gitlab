...
| `use_inherited_settings` | boolean | no | Indicates whether to inherit the default settings. Defaults to `false`. |
| `vulnerabilities_enabled` | boolean | no | Available only in GitLab EE. If set to `true`, creates Jira issues for GitLab vulnerabilities.|
...
| `customize_jira_issue_enabled` | boolean | no | Available only in GitLab EE. When set to `true`, will navigate to a prefilled form on the Jira instance when creating a Jira issue from a vulnerability. |
