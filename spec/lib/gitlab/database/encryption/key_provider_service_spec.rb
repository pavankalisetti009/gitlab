...
RSpec.describe Gitlab::Database::Encryption::KeyProviderService, feature_category: :database do
  ...
  describe 'supported key providers' do
    ...
    where(:key_type, :expected_key_provider_class, :expected_keys) do
      :db_key_base | Gitlab::Database::Encryption::NonDerivedKeyProvider | wrap_secrets(Settings.db_key_base_keys)
      :db_key_base_truncated | Gitlab::Database::Encryption::NonDerivedKeyProvider | wrap_secrets(Settings.db_key_base_keys_32_bytes)
      :db_key_base_32 | Gitlab::Database::Encryption::NonDerivedKeyProvider | wrap_secrets(Settings.db_key_base_keys_truncated)
    ...
    with_them do
      subject(:service) { described_class.new(key_type) }
