...
module QA
  module EE
    module Resource
      class InstanceExternalStreamingDestination < QA::Resource::Base
        ...
        # @return [InstanceExternalStreamingDestination, nil] the destination or nil if not found
        def self.find_by_name(name:)
          instance = new
          destinations = instance.all
        ...
        def gid
          "gid://gitlab/AuditEvents::Instance::ExternalStreamingDestination/#{id}"
        end
        ...
        # All audit event streaming destinations for the instance
        #
        ...
        def all
          ...
          response.dig(:audit_events_instance_streaming_destinations, :nodes) || []
        end
        ...
        def api_post_body
          <<~GQL
            mutation {
              instanceAuditEventStreamingDestinationsCreate(input: { #{mutation_params} }) {
                errors
        ...
        def api_delete_body
          <<~GQL
            mutation {
              instanceAuditEventStreamingDestinationsDelete(input: { id: "#{gid}" }) {
                errors
              }
            }
          GQL
